...
GRANT EXECUTE TASK, EXECUTE MANAGED TASK ON ACCOUNT TO ROLE fs_admin;
CREATE OR REPLACE TASK increment_weather_data
	schedule='USING CRON * * * * * America/Los_Angeles'
	AS
    INSERT INTO airport_weather_station
    SELECT DATEADD(MINUTE, 1, datetime_utc) AS datetime_utc, airport_zip_code, rain_mm_h
    FROM airport_weather_station
    WHERE datetime_utc = (SELECT MAX(datetime_utc) FROM airport_weather_station);
ALTER TASK increment_weather_data RESUME;


SELECT * FROM us_flight_schedules ORDER BY scheduled_departure_utc;
SELECT * FROM airport_weather_station ORDER BY datetime_utc;
select * FROM plane_model_attributes;

SELECT COUNT(*) FROM airport_weather_station;
